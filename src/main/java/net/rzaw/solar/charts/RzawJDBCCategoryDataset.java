...
/**
 * A {@link CategoryDataset} implementation over a database JDBC result set. The dataset is populated via a call to
 * {@link #executeQuery(String)} with the string SQL query. The SQL query must return at least two columns. The first
 * column will be the category name and remaining columns values (each column represents a series). Subsequent calls to
 * {@link #executeQuery(String)} will refresh the dataset.
 * <p>
 * The database connection is read-only and no write back facility exists.
 * <p>
 * NOTE: Many people have found this class too restrictive in general use. For the greatest flexibility, please consider
 * writing your own code to read data from a <code>ResultSet</code> and populate a {@link DefaultCategoryDataset}
 * directly.
 */
public class RzawJDBCCategoryDataset
    extends DefaultCategoryDataset
{
     ...
     */
    public RzawJDBCCategoryDataset( Connection connection )
    {
        if ( connection == null )
        {
    ...
    /**
     * Creates a new dataset with the given database connection, and executes the supplied query to populate the
     * dataset.
     * 
     ...
     */
    public RzawJDBCCategoryDataset( Connection connection, String query )
        throws SQLException
    {
        this( connection );
        executeQuery( query );
    ...
    /**
     * Populates the dataset by executing the supplied query against the existing database connection. If no connection
     * exists then no action is taken.
     * <p>
     * The results from the query are extracted and cached locally, thus applying an upper limit on how many rows can be
